...
const uint8_t HOSTILES[] = { (uint8_t)NPCType::Enemy, (uint8_t)NPCType::Skeleton,
                             ...
                             (uint8_t)NPCType::FireElemental, (uint8_t)NPCType::StoneElemental };
...
const char* questEnemyLabel(uint8_t npcType) {
    switch ((NPCType)npcType) {
        ...
        case NPCType::FireElemental:  return "Fire Elemental";
        case NPCType::StoneElemental: return "Stone Elemental";
        default:                   return "Foe";
    }
}
...
std::vector<Quest> buildTownQuests(uint32_t seed, int townIndex,
    ...
    for (int s = 0; s < slots; ++s) {
        ...
        // Some quests are simple scouting trips — travel out and find the place.
        bool explore = !boss && (pick(5) == 0);
        ...
        if (tgt.isTown) {
            ...
            q.requiredCount = 1;
            q.title = std::string("Scout ") + tgt.name;
            q.text  = "Travel to " + tgt.name + " and scout it for the townsfolk.";
        } else if (kill) {
        ...
        } else if (explore) {                         // a quick but fair payout for the trek
