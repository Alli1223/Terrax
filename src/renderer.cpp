...
void Renderer::renderWorld(AppContext& ctx, GLFWwindow* window, float currentTime) {
    ...
    float lanternIntensity = (ctx.lanternHeld ? 1.2f : 0.55f) * flicker;
    float lanternRadius    =  ctx.lanternHeld ? 14.0f : 6.0f;
    glm::vec3 fwdVec  = glm::vec3(sinf(glm::radians(ctx.playerYaw)), 0.0f,  cosf(glm::radians(ctx.playerYaw)));
    glm::vec3 rightVec= glm::vec3(cosf(glm::radians(ctx.playerYaw)), 0.0f, -sinf(glm::radians(ctx.playerYaw)));
    glm::vec3 lanternPos = ctx.lanternHeld
        ? ctx.camera.position + glm::vec3(0.0f, 1.35f, 0.0f) - rightVec * 0.28f + fwdVec * 0.15f
        : ctx.camera.position + glm::vec3(0.0f, 0.78f, 0.0f) - rightVec * 0.22f + fwdVec * 0.12f;
